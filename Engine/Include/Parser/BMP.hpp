...
#pragma pack(pop)
    ...
    {
    public:
        virtual Entropy::Image Parse(const Buffer& buf)
        {
	        Entropy::Image img;
                ...
                if (img.bitcount < 24) {
                    ...
                    for (int32_t y = img.Height - 1; y >= 0; y--) {
                        for (uint32_t x = 0; x < img.Width; x++) {
                            (img.data + img.Width * (img.Height - y - 1) + x)->data[0] = *(pSourceData + img.pitch * y + x * (img.bitcount >> 3));
                            (img.data + img.Width * (img.Height - y - 1) + x)->data[1] = *(pSourceData + img.pitch * y + x * (img.bitcount >> 3) +1 );
                            (img.data + img.Width * (img.Height - y - 1) + x)->data[2] = *(pSourceData + img.pitch * y + x * (img.bitcount >> 3) +2 );
                            (img.data + img.Width * (img.Height - y - 1) + x)->data[3] = *(pSourceData + img.pitch * y + x * (img.bitcount >> 3) +3 );
                        }
                    }
                }
            }
